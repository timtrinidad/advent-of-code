...
{:ok, input} = File.read('2022/10/input.txt')

{history, _} = DayTen.generate_cycle_history(input)
frequency_cycles = [20, 60, 100, 140, 180, 220]
for n <- frequency_cycles do history |> Enum.at(n - 1) |> Kernel.*(n) end
  |> Enum.sum |> IO.inspect(label: "Sum of register values at frequencies [#{Enum.join(frequency_cycles, ", ")}]")
DayTen.render(history)
