...
defmodule DayFourteen do
  ...
  def count_sand(input, is_solid_floor) do
    ...
    set = MapSet.new()
    set = wall_paths |> Enum.reduce(set, fn [{start_x, start_y}, {end_x, end_y}], set ->
  ...
  def place_grain(set, {curr_x, curr_y}, floor, is_solid_floor, num_grains) do
    ...
    cond do
      ...
      curr_y < floor && down_empty? -> place_grain(set, {curr_x, curr_y + 1}, floor, is_solid_floor, num_grains)
      curr_y < floor && diag_left_empty? -> place_grain(set, {curr_x - 1, curr_y + 1}, floor, is_solid_floor, num_grains)
      curr_y < floor && diag_right_empty? -> place_grain(set, {curr_x + 1, curr_y + 1}, floor, is_solid_floor, num_grains)
      # If we've reached the abyss, stop dropping sand (base case)
      !is_solid_floor && curr_y == floor -> {set, num_grains}
      # If the floor is solid and we've reached it, stop moving sand. Start dropping a new one.
      is_solid_floor && curr_y == floor -> map = MapSet.put(set, {curr_x, floor}); place_grain(map, {500, 0}, floor, is_solid_floor, num_grains + 1)
      # If the entrance is blocked, stop dropping sand (base case)
      MapSet.member?(set, {500, 0}) -> {set, num_grains}
      # Grain of sand has stopped moving, no available moves. Drop a new grain.
      true -> set = MapSet.put(set, {curr_x, curr_y}); place_grain(set, {500, 0}, floor, is_solid_floor, num_grains + 1)
    end
  end
end
...
{_, num_grains} = DayFourteen.count_sand(input, false);
num_grains |> IO.inspect(label: "# grains without floor")
{_, num_grains} = DayFourteen.count_sand(input, true)
