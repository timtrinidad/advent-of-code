...
aoc 2022, 12 do
  ...
  def get_paths(input) do
    ...
    vertices =
      Enum.flat_map(1..height, fn y ->
        Enum.map(1..width, fn x -> coord_to_key({x - 1, y - 1}) end)
      end)
      |> Enum.reduce(%{}, fn point, acc ->
        Map.put(acc, point, :digraph.add_vertex(graph, point))
      end)
    ...
    nodes =
      ...
      |> Enum.flat_map(fn {row, y} ->
        ...
        |> Enum.map(fn {node, x} ->
          ...
          Enum.reduce([{1, 0}, {0, 1}, {-1, 0}, {0, -1}], [], fn {delta_x, delta_y}, _ ->
            neighbor_xy = {x - delta_x, y - delta_y}
            neighbor = (elevation_map |> Enum.at(y - delta_y) || []) |> Enum.at(x - delta_x)

            valid_neighbor =
              neighbor && elem(neighbor_xy, 0) >= 0 && elem(neighbor_xy, 1) >= 0 &&
            ...
            if valid_neighbor,
              do:
                :digraph.add_edge(
                  graph,
                  vertices[coord_to_key({x, y})],
                  vertices[coord_to_key(neighbor_xy)]
                )
    ...
    |> Enum.map(fn {node, node_xy} ->
      steps =
        :digraph.get_short_path(
          graph,
          vertices[coord_to_key(node_xy)],
          vertices[coord_to_key(end_xy)]
        )
