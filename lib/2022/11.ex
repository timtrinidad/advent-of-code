...
aoc 2022, 11 do
  def p1(input) do
    num_inspections_sorted =
      simulate_rounds(input, 20, 3)
      |> Enum.map(fn x -> x["num_inspections"] end)
      |> IO.inspect(charlists: :as_lists, label: "# Inspections per monkey")
      |> Enum.sort()
      |> Enum.reverse()

    IO.inspect(Enum.at(num_inspections_sorted, 0) * Enum.at(num_inspections_sorted, 1),
      label: "Monkey business level"
    )
  end

  def p2(input) do
    num_inspections_sorted =
      simulate_rounds(input, 10000, 1)
      |> Enum.map(fn x -> x["num_inspections"] end)
      |> IO.inspect(charlists: :as_lists, label: "# Inspections per monkey")
      |> Enum.sort()
      |> Enum.reverse()
  ...
  def exec_inspection(monkeys, worry_divisor, [curr_monkey_id | next_monkey_ids]) do
    ...
    cond do
      ...
      true ->
        ...
        to_monkey_id =
          if rem(curr_item, curr_monkey["test_divisible_by"]) == 0 do
