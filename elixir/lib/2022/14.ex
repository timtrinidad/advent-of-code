...
aoc 2022, 14 do
  def p1(input) do
    {map, num_grains} = count_sand(input, false)
    render_map(map)
    num_grains |> IO.inspect(label: "# grains without floor")
  end

  def p2(input) do
    {map, num_grains} = count_sand(input, true)
    render_map(map)
  ...
  def count_sand(input, is_solid_floor) do
    ...
    wall_points =
      ...
      |> Enum.map(fn [first_point | path] ->
        path
        |> Enum.flat_map_reduce(first_point, fn {point_x, point_y}, {previous_x, previous_y} ->
          # Traverse each vertext outputting individual coordinates
          points =
            for x <- previous_x..point_x do
  ...
  def place_grain(map, {curr_x, curr_y}, floor, is_solid_floor, num_grains) do
    ...
    cond do
      ...
      curr_y < floor && down_empty? ->
        place_grain(map, {curr_x, curr_y + 1}, floor, is_solid_floor, num_grains)

      curr_y < floor && diag_left_empty? ->
        place_grain(map, {curr_x - 1, curr_y + 1}, floor, is_solid_floor, num_grains)
      ...
      true ->
        map = put_in(map, [curr_y, curr_x], "o")
        place_grain(map, {500, 0}, floor, is_solid_floor, num_grains + 1)
    end
  end
  ...
  def render_map(map) do
    ...
    {x_min, x_max} =
      map |> Map.values() |> Enum.flat_map(fn row -> row |> Map.keys() end) |> Enum.min_max()
